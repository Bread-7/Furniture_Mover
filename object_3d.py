...
class Object3D:
    def __init__(self, render, vertices_arr):
        ...
        self.largest_face = 0
        self.closest_vertices = np.array([])
        self.farthest_vertices = np.array([])
        self.invisible_pts = np.array([])
    ...
    def screen_projection(self, cam_pos):
        ...
        vertex_colors = [pg.Color('red'), pg.Color('blue'), 
        ...
        triangle_culling = self.triangle_culling(cam_pos)
        self.invisible_pts = triangle_culling[0]
        self.invisible_pts_indices = triangle_culling[1]
        # print('type', self.invisible_pts[0][0], type(self.invisible_pts[0][0]))
        areas = np.array([])
        polygons = np.array([], dtype=object)
        for face in self.invisible_pts:
            # temp_arr = np.intersect1d(np.where(np.isin(self.invisible_pts, face))[0], np.arange(len(face)))
            # if not np.isin(face, self.invisible_pts).any():    
        ...
        self.largest_face = np.array(areas).argmax()
        # self.closest_vertices = self.vertices[self.faces[self.largest_face]]
        # self.farthest_vertices = self.vertices[self.faces[self.largest_face + 1]] if self.largest_face != 5 else self.vertices[self.faces[self.largest_face - 1]]
        # self.invisible_pts = self.plane_3D_to_2D(cam_pos)
        # for coord in self.invisible_pts:
        #     invisible_pts = np.append(invisible_pts, np.intersect1d(np.where(np.isin(self.vertices, coord))[0], np.arange(len(coord))))

        for index, polygon in enumerate(polygons):
            # (not set(invisible_pts).intersection(set(polygon))) and 
            if ((not np.any((polygon == self.render.H_WIDTH) | (polygon == self.render.H_HEIGHT)))):
                # if(index == self.largest_face):
                pygame.gfxdraw.filled_polygon(self.render.screen, polygon, vertex_colors[self.invisible_pts_indices[index]])
        ...
        for i, vertex in enumerate(vertices):
            # (not vertex in invisible_pts) and 
            if ((not np.any((vertex == self.render.H_WIDTH) | (vertex == self.render.H_HEIGHT)))):
                if i in self.invisible_pts:
                    pg.draw.circle(self.render.screen, pg.Color('red'), vertex, 3)
                # if(vertices[3][0] == vertex[0] and vertices[3][1] == vertex[1]):
                #     pg.draw.circle(self.render.screen, pg.Color('brown'), vertex, 3)
    ...
    def triangle_culling(self, cam_pos):
    ...
    def rotate_x_obj(self, theta):
        ...
        self.x_theta = (self.x_theta + theta) % (np.copysign(2 * np.pi, self.x_theta))
        if self.x_theta > np.pi:
            self.x_theta -= 2 * np.pi
        elif self.x_theta < -np.pi:
            self.x_theta += 2 * np.pi
        self.update_center()     

    def rotate_y_obj(self, theta):
        self.vertices = self.vertices @ rotate_y(theta)
        self.y_theta = (self.y_theta + theta) % (np.copysign(2 * np.pi, self.y_theta))
        if self.y_theta > np.pi:
            self.y_theta -= 2 * np.pi
        elif self.y_theta < -np.pi:
            self.y_theta += 2 * np.pi
        self.update_center()

    def rotate_z_obj(self, theta):
        self.vertices = self.vertices @ rotate_z(theta)
        self.z_theta = (self.z_theta + theta) % (np.copysign(2 * np.pi, self.z_theta))
        if self.z_theta > np.pi:
            self.z_theta -= 2 * np.pi
        elif self.z_theta < -np.pi:
            self.z_theta += 2 * np.pi
        self.update_center()
